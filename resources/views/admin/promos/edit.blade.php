...
@section('title', 'Edit Discount Code - Admin')
...
<div>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-4xl font-bold text-gray-900">Edit Discount Code</h1>
        <p class="text-gray-600 mt-2">Update the promotional offer details</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <form method="POST" action="{{ route('admin.promos.update', $promo) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <!-- Usage Stats -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div>
                        <p class="text-sm text-gray-600">Times Used</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $promo->times_used }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Created</p>
                        <p class="text-sm font-medium text-gray-900">{{ $promo->created_at->format('M d, Y') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Last Updated</p>
                        <p class="text-sm font-medium text-gray-900">{{ $promo->updated_at->format('M d, Y H:i') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="discount_type" value="percentage" {{ old('discount_type', $promo->discount_type) === 'percentage' ? 'checked' : '' }}
                                    onchange="toggleDiscountType()"
                                    class="w-4 h-4 text-orange-600">
                                <span class="text-gray-700 font-medium">Percentage Off</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="discount_type" value="fixed_amount" {{ old('discount_type', $promo->discount_type) === 'fixed_amount' ? 'checked' : '' }}
                                    onchange="toggleDiscountType()"
                                    class="w-4 h-4 text-orange-600">
                    ...
                    <div id="percentage-input" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <input type="number" name="discount_percentage" value="{{ old('discount_percentage', $promo->discount_percentage) }}"
                                    min="1" max="100"
                    ...
                    <!-- Fixed Amount Input -->
                    <div id="fixed-input" class="grid grid-cols-1 md:grid-cols-2 gap-6 {{ $promo->discount_type === 'fixed_amount' ? '' : 'hidden' }}">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Fixed Discount Amount <span class="text-red-500">*</span></label>
                            <div class="flex items-center gap-2">
                                <span class="text-gray-700 font-medium">₵</span>
                                <input type="number" name="discount_amount" value="{{ old('discount_amount', $promo->discount_amount) }}"
                                    min="0.01" step="0.01"
                                    class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-gray-700 font-medium">₵</span>
                            <input type="number" name="minimum_order_value" value="{{ old('minimum_order_value', $promo->minimum_order_value) }}"
                                min="0" step="0.01"
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                        </div>
            ...
            <div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="applicable_to" value="all" {{ old('applicable_to', $promo->applicable_to) === 'all' ? 'checked' : '' }}
                                    onchange="toggleServiceSelect()"
                                    class="w-4 h-4 text-orange-600">
                                <span class="text-gray-700">All Services</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="applicable_to" value="first_booking" {{ old('applicable_to', $promo->applicable_to) === 'first_booking' ? 'checked' : '' }}
                                    onchange="toggleServiceSelect()"
                                    class="w-4 h-4 text-orange-600">
                                <span class="text-gray-700">First Booking Only</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="applicable_to" value="specific_service" {{ old('applicable_to', $promo->applicable_to) === 'specific_service' ? 'checked' : '' }}
                                    onchange="toggleServiceSelect()"
                                    class="w-4 h-4 text-orange-600">
                    ...
                    <!-- Service Select -->
                    <div id="service-select" class="{{ $promo->applicable_to === 'specific_service' ? '' : 'hidden' }}">
                        ...
                        <select name="service_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                            ...
                            @foreach($services as $service)
                                <option value="{{ $service->service_id }}" {{ old('service_id', $promo->service_id) == $service->service_id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="expires_at" value="{{ old('expires_at', $promo->expires_at->toDateString()) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 text-xs mt-1">Current usage: {{ $promo->times_used }} / {{ $promo->usage_limit ?? 'Unlimited' }}</p>
                        @error('usage_limit')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="active" {{ old('status', $promo->status) === 'active' ? 'checked' : '' }}
                            class="w-4 h-4 text-orange-600">
                        <span class="text-gray-700 font-medium">Active</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="inactive" {{ old('status', $promo->status) === 'inactive' ? 'checked' : '' }}
                            class="w-4 h-4 text-orange-600">
